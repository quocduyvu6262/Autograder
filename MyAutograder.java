...
/**
 * Represents the MyAutograder Ticket System with a Ticket Priority Queue,
 * Resolved Tickets stack and other Queue Statistics
 */
public class MyAutograder extends javax.swing.JFrame {
    ...
    public void setTicketTypePriority(){
        ...
        orderMap.put(Ticket.ENVIRONMENT_SETUP, MyAutograder.FIRST_PRIORITY);
        orderMap.put(Ticket.DEBUGGING, MyAutograder.SECOND_PRIORITY);
        orderMap.put(Ticket.CONCEPT_DOUBTS, MyAutograder.THIRD_PRIORITY);
        orderMap.put(Ticket.OTHERS, MyAutograder.FOURTH_PRIORITY);
        Ticket.setOrderMap(orderMap);
    }
    
    
    private void startClock(){
        clockTime = new Timer(MyAutograder.UPDATE_CLOCK_DELAY, updateClockAction);
        clockTime.start(); 
    }
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if(status == MyAutograder.BUSY){
                ...
                ticketsResolvedLabel.setText(String
                        .format(MyAutograder.TICKETS_RESOLVED_MESSAGE, 
                                ++ticketsResolved));
            }
            status = MyAutograder.AVAILABLE;
            printQueue();
    ...
    /**
     * Add one ticket to queue
     */
    public void addTicket(String name, String type) {
        Ticket ticket = new Ticket(name,type);
        ticket.setCreatedAt(seconds);
        ticket.setTicketNumber(ticketNumber+1);
    ...
    private JPanel createTicketPanel(Ticket ticket){
        ...
        JLabel studentName = new JLabel(String.format(MyAutograder
                .STUDENT_NAME_LABEL,ticket.getStudentName()));
        JLabel ticketStatus = new JLabel(String.format(MyAutograder
                .STATUS_LABEL, ticket.getTicketStatus())); 
        JLabel ticketType = new JLabel(String.format(MyAutograder
                .TYPE_LABEL, ticket.getTicketType())); 
        JLabel ticketCreatedNumber = new JLabel(String.format(MyAutograder
    ...
    private JPanel createResolvedTicketPanel(Ticket ticket){
        ...
        JLabel ticketCreated = new JLabel(String
                .format(MyAutograder.TICKET_CREATED_AT_LABEL, 
                    ticket.getCreatedAt()));
        ...
        JLabel ticketResolved = new JLabel(String
                .format(MyAutograder.TICKET_RESOLVED_AT_LABEL, 
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if(numESTickets>0){
                ESTimeLabel.setText(String
                        .format(MyAutograder.SECONDS_UNIT,
                                sumEnvironmentWaitTime/numESTickets));
            }
            if(numDebugTickets>0){
                DebugTimeLabel.setText(String.format(MyAutograder.SECONDS_UNIT,
                        sumDebuggingWaitTime/numDebugTickets));
            }
            if(numConceptTickets>0){
                ConceptTimeLabel.setText(String.format(MyAutograder.SECONDS_UNIT,
                        sumConceptWaitTime/numConceptTickets));
            }
            if(numOthersTickets>0){
                OthersTimeLabel.setText(String.format(MyAutograder.SECONDS_UNIT,
    ...
    private void startButtonActionPerformed(java.awt.event.ActionEvent evt) {                                            
        ...
        Timer addTicketTimer = 
                new Timer(MyAutograder.ADD_TICKET_DELAY, addTicketsAction);
        Timer processTicketTimer = 
                new Timer(MyAutograder.PROCESS_TICKET_DELAY,
                        processTicketsAction); 
        Timer removeTicketTimer = 
                new Timer(MyAutograder.REMOVE_TICKET_DELAY, removeTicketsAction);
        Timer updateStats = 
                new Timer(MyAutograder.UPDATE_STATISTICS_DELAY, updateStatistics);
        updateStats.start();
        addTicketTimer.start();
    }           
    
    /**
     * Create the input form for the ticket
     */
    private void inputFormActionPerformed(java.awt.event.ActionEvent evt){
        startButton.setEnabled(false);
        String[] options = {Ticket.ENVIRONMENT_SETUP, Ticket.DEBUGGING, Ticket.CONCEPT_DOUBTS, Ticket.OTHERS};
        /* set up layout */
        JDialog jdialog = new JDialog();
        JLabel jlabel = new JLabel("Ticket Number 1");
        JTextField nameInput = new JTextField("Your Name");
        final JComboBox<String> typeInput = new JComboBox<String>(options);
        typeInput.setSelectedItem(Ticket.ENVIRONMENT_SETUP);
        JLabel nameLabel = new JLabel("Enter your name: ");
        JLabel typeLabel = new JLabel("Environment type: ");
        JPanel panelWest = new JPanel();
        JPanel panelCenter = new JPanel();

        JPanel jpanelButton = new JPanel();
        JPanel jpanelTicketNum = new JPanel();
        /* set element size */
        panelWest.setLayout(new GridLayout(4, 1));
        panelCenter.setLayout(new GridLayout(4, 1));
        jdialog.setLayout(new BorderLayout());
        jdialog.setBounds(500,300,400,300);
        jdialog.setResizable(false);
        nameInput.setPreferredSize(new Dimension(10,10));
        ...
        submitButton.addActionListener(new ActionListener(){
            @Override
            public void actionPerformed(ActionEvent e) {
                //TODO: submit new ticket and close dialog
                String name = nameInput.getText();
                String typeQuestion = (String) typeInput.getEditor().getItem();
                if(typeQuestion.compareTo("")==0) typeQuestion = Ticket.ENVIRONMENT_SETUP;
                addTicket(name,typeQuestion);
                startButton.setEnabled(true);
                jdialog.setVisible(false);
            }   
        });
        /* locate element in the dialog */

        //add stuff to jpanel
        jpanelButton.add(cancelButton);
        jpanelButton.add(submitButton);
        jdialog.add(jpanelButton,BorderLayout.SOUTH);

        jpanelTicketNum.add(jlabel);
        jdialog.add(jpanelTicketNum,BorderLayout.NORTH);

        panelWest.add(nameLabel);
        panelCenter.add(nameInput);
        panelWest.add(typeLabel);
        panelCenter.add(typeInput);

        jdialog.add(panelWest,BorderLayout.WEST);
        jdialog.add(panelCenter,BorderLayout.CENTER);

        jdialog.setVisible(true);
    }
    /**
     * Process Ticket progress
     * @param args
     */
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {         
                MyAutograder ag = new MyAutograder();
                ag.setVisible(true);                         
            }
        });
    } 
    ...
    private void initComponents() {
        ...
        setTitle("MyAutograder Simulation");
        ...
        autograderSimulationLabel.setText("MyAutograder Simulation");
        /**------------------------------------custom start button--------------------- */
            ...
            .TRAILING, layout.createSequentialGroup()
                ...
                .swing.GroupLayout.Alignment.TRAILING)
                    .addGroup(layout.createSequentialGroup()
                        ...
                        .addGap(18, 18, 18)
                        .addComponent(startButton, /* will change */
                        javax.swing.GroupLayout.PREFERRED_SIZE,
